...
class HabitViewController: UIViewController {
    ...
    @objc private func editTapped() {
        if let viewModel = self.viewModel {
            if let data = viewModel.getHabit(indentifier: viewModel.getHabitIdentifier()) {
                let controller = NewHabitController()

                controller.editedHabit = HabitBiding(identifier: data.identifier, title: data.title, goal: data.goal, startDate: data.startDate, reminders: data.reminders, imageID: data.imageID, repetition: data.repetition)
